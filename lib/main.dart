...
void main() {
  runApp(InstituteConfigurationApp());
}

class InstituteConfigurationApp extends StatelessWidget {
  @override
...
class _CampusTabState extends State<CampusTab> {
  ...
  Future<void> updateCampus() async {
    ...
    {
      ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Campus ID and Campus Name Required'),
       backgroundColor: Colors.redAccent,),
    );
    return;
    } 
  ...
  Future<void> deleteCampus(int index) async {
    ...
    if (confirm == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Campus deleted successfully') ,backgroundColor: Colors.blueAccent,),
      );
    }
  }
